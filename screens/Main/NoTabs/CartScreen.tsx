...
const PlaceOrderCard = ({fetchUserInfo})=>{
...
const [cartAmount,setCartAmount] = useState(0);
  ...
  try{
    ...
    const req = await fetchUserInfo();
    if(req.success){
       setCartAmount(req.user.totalCartAmount);
    }
  ...
  useEffect(()=>{
    ...
    return (
        <View style = {{width , alignItems  : 'center' , backgroundColor : '#f4f4f4ff' , elevation : 3, flexDirection : 'row'}}>
            <View style = {{width : width*0.47 , alignItems : 'center' , padding : 10}}>
                <Text>{cartAmount}</Text>
            </View>

<View style = {{width : width*0.47 , padding : 10}}>
    <TouchableOpacity style = {{alignItems : 'center' , height  : 40, justifyContent : 'center', gap : 4, padding : 2, flexDirection : 'row' , borderRadius : 10 , backgroundColor : '#f6e529ff'}}>
...
const CartCard = ({ cart, onPress ,deleteCart , setCarts}) => {
  ...
  const netAmount = Array.isArray(pricingList)
    ...
    const handleDelete = async ()=>{
      ...
      try{
        ...
        const response = await deleteCart(cart._id);
        if(response.success){
...
const CartListScreen = () => {
  ...
  const fetchCarts = userStore((state) => state.fetchCarts);
  const deleteCart = userStore((state)=>state.deleteCart);
  ...
  const fetchCartsList = useCallback(
    async (pageNumber = 1, reset = false) => {
      ...
      try {
        const response = await fetchCarts(pageNumber, PAGE_LIMIT);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#f3f4f6' }}>
      ...
      <PlaceOrderCard  fetchUserInfo = {fetchUserInfo}/>
    </View>
  );
};
